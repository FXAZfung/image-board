...
var StartCmd = &cobra.Command{
	Use:   "start",
	Short: "Silent start image-board server with `--force-bin-dir`",
...
func start() {
	...
	if pid != -1 {
		...
		if err == nil {
			log.Info("image-board already started, pid ", pid)
			return
		}
	}
	args := os.Args
	...
	if err != nil {
		log.Warn("failed to record pid, you may not be able to stop the program with `./image-board stop`")
	}
}
